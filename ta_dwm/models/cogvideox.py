...
from diffusers.models.attention_processor import CogVideoXAttnProcessor2_0, AttentionProcessor
...
class CogVideoXTransformer3DModel_wTraj(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # target latent
        encoder_hidden_states: torch.Tensor, # history latent
        ...
        history_num_frames = encoder_hidden_states.shape[1]
        if not only_fut: # padding for history + future
        ...
        encoder_hidden_states = hidden_states[:, :cond_seq_length] # traj embed
        ...
        if p_t is None:
            if only_fut:
                num_frames = num_frames + history_num_frames
            ...
            if only_fut:
                output = output[:, history_num_frames:, :, :, :]
        else:
