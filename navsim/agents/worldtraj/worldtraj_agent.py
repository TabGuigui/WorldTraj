...
from .worldtraj_rewarder import TrajWorldRefiner
...
class WorldTrajAgent(AbstractAgent):
    ...
    def initialize(self, from_world_model=False) -> None:
        ...
        if from_world_model: # training, weights from TA-DWM
            ...
            if self._worldmodel_checkpoint_path:
                ...
                for key, val in worldmodel_state_dict.items():
                    ...
                    if "traj_encoder" in adapt_key: 
                        adapt_key = adapt_key.replace("traj_encoder.", "trajencoder_wm_high.")
                        state_dict[adapt_key] = val
                    if "adapters" in adapt_key: 
                        adapt_key = adapt_key.replace("adapters.a", "adapters_wm_high.a")
                        state_dict[adapt_key] = val
    ...
    def forward_wm(self, features, targets, unique_tokens):
        ...
        # indices = torch.cat([indices[:, -final_topk+1:], indices[:, :1]], dim=1) 
        ...
        pos_traj, future_scene_embed, refine_loss, refine_weight = self.trajrefiner(topk_traj, topk_traj_embed, visual_token, [rewards_matrix])
        if dist.get_rank() == 0 and self.training:
            print(refine_weight[0])
        ...
        # latent_consist_loss = 0

        loss = {}
        loss["re_bt_loss"] = refine_loss
        loss["re_consist_loss"] = latent_consist_loss
        loss["loss"] = latent_consist_loss +  loss["re_bt_loss"]
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None, token_list=None, eval=False) -> Dict[str, torch.Tensor]:
        ...
        # --------evaluation-----------
            ...
            if self.train_mode == "wm":
                ...
                # visualization 
                # with torch.no_grad():
                #     ori_visual_latents = self.adapters_wm(features["low_visual_token"][None])
                #     h, w = ori_visual_latents.shape[-2:]
                #     shape = (bs, self.cfg.predict_frames // self.cfg.temporal_compression_ratio + 2 + 1, self.cfg.vae_embed_dim, h, w)
                #     rand_latents = randn_tensor(shape, generator=None, device=traj_score.device, dtype=traj_score.dtype)
                #     print("pos future")

                #     pos_latents = self.world_model_refine(ori_visual_latents, pos_traj, latents=rand_latents, timesteps=100)
                #     pos_latents = pos_latents.permute(0, 2, 1, 3, 4)
                #     cond, preds = pos_latents[:, :, :2], pos_latents[:, :, 2:]
                #     latents = 1 / self.vae_backbone.config.scaling_factor * cond
                #     frames = self.vae_backbone.decode(latents[:1]).sample
                #     frames = torch.clamp(frames, -1, 1)
                #     frames = frames.permute(0, 2, 3, 4, 1)
                #     pred = ((frames[0].cpu().detach().numpy() / 2 + 0.5) * 255).astype('uint8')
                #     for i in range(pred.shape[0]):
                #         cv2.imwrite(f'cond_{i}.jpg', pred[i, :, :, ::-1])
                ...
                #     pred = ((frames[0].cpu().detach().numpy() / 2 + 0.5) * 255).astype('uint8')
                #     for i in range(pred.shape[0]):
                #         cv2.imwrite(f'visual_eccv_supp/pred_{i}.jpg', pred[i, :, :, ::-1])

                    
                #     print("top1 future")
                #     pos_latents = self.world_model_refine(ori_visual_latents, topk_traj[:, 0], latents=rand_latents, timesteps=100)
                #     pos_latents = pos_latents.permute(0, 2, 1, 3, 4)
                #     cond, preds = pos_latents[:, :, :2], pos_latents[:, :, 2:]
                #     # latents = 1 / self.vae_backbone.config.scaling_factor * cond
                #     # frames = self.vae_backbone.decode(latents[:1]).sample
                ...
                #     pred = ((frames[0].cpu().detach().numpy() / 2 + 0.5) * 255).astype('uint8')
                #     for i in range(pred.shape[0]):
                #         cv2.imwrite(f'visual_eccv_supp/pred_{i}_worefine.jpg', pred[i, :, :, ::-1])
                    
                    
                #     print("expert future")
                #     expert_latents = self.world_model_refine(ori_visual_latents, targets["trajectory"][None].to(torch.float), latents=rand_latents, timesteps=100)
                #     expert_latents = expert_latents.permute(0, 2, 1, 3, 4)
                #     cond, preds = expert_latents[:, :, :2], expert_latents[:, :, 2:]
                #     latents = 1 / self.vae_backbone.config.scaling_factor * preds
                #     frames = self.vae_backbone.decode(latents[:1]).sample
                #     frames = frames.permute(0, 2, 3, 4, 1)
                #     frames = torch.clamp(frames, -1, 1)
                #     pred = ((frames[0].cpu().detach().numpy() / 2 + 0.5) * 255).astype('uint8')
                #     for i in range(pred.shape[0]):
                #         cv2.imwrite(f'visual_eccv_supp/pred_{i}_expert.jpg', pred[i, :, :, ::-1])
